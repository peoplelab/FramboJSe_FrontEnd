...
/***************************************************************************************************/
	...
	OBSOLETO	OBSOLETO	OBSOLETO	OBSOLETO	OBSOLETO	OBSOLETO	OBSOLETO	OBSOLETO
	...
	function render_snippet(params) {
		...
		function snippetsCallBack(params) {
			...
			switch (snippet) {
				...
				case 'dd_custom':																		// Dropdown: nr. anni, prodotti, materie prime
					_isDDReady = true;
				...
				case 'input_float':
                    ...
                    var $percTFR = $('#PercTFR');
                    var $_visiblePercTFR = $('._visiblePercTFR');
					var valTfr = $percTFR.val() ? $percTFR.val() / 1000 : 0;					// Valore pct. normalizzato
					$_visiblePercTFR.val(valTfr);													// Imposta campo di input col valore corrente
					...
					$('[name=tfrInAzienda]').click(function(){
						...
						switch (id) {
							...
							case 'TFR3':
							case '_visiblePercTFR':
								$('#TFR3Label').removeClass('inactive');
								break;
						}
					});

					$('._visiblePercTFR')																// Gestione eventi campo IVA custom
					.focusin(function(){
						$('#TFR3Label').removeClass('inactive');
					...
					// ** 2: Impostazione campo IVA **
					var valIva = $('#IvaDefault').val() ? $('#IvaDefault').val() / 1000 : 0;
					$('._visibleIvaDefault').val(valIva);

					$('._visibleIvaDefault').change(function() {
						...
						if (isNaN(v) || v == null || v == '') {
							v = 0;
							$('._visibleIvaDefault').val(v);
						}
						if (v > max || v < min) {
							v = (v > max)? max : min;
							$('._visibleIvaDefault').val(v);
						}
						$('#IvaDefault').val(v * 1000);
					});
