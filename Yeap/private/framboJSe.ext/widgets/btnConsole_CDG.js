//----------------------------------------------------------------------------------------
// File  : btnConsole_CDG.js
//
// Desc  : Costruzione della console dei pulsante gestione pagg. Controlo di gestione
// Path  : /Private/framboJse.ext/widgets
// target:  <widget_btnConsole_CDG />
// output:  widget
//----------------------------------------------------------------------------------------
define([
	'dd_base',
//	'/private/modules/economics/model/economics.model.js',
	'/private/modules/controllo/model/controllo.model.js',
], function (base, model) {

	var _ITEMNAME = 'btnConsole_CDG';
	...
	//	BuildHtml: EX_buildHtml,																			// Item's HTML
	...
	function buildHtml(tagPars, pbAttrs) {
		...
		//	page : 'menu',
			...
			var CDG    = eval(data.find('Status').text());												// Status Business Plan: 0 = vuoto, 1 = salvato, 2 = compilato, 3 = inviato, 4 = elaborato (ok Leanus), 99 = errore
			var gestID = eval(data.find('TipoGestionale').text());										// Salva le info sul tipo gestionale per l'import consuntivi (utilizzato per determinare il tipo cons.)
		...
		//	var CDG         = '*';
			var btnSave     = false;
			var btnImporta  = false;
			var btnModifica = false;
			...
			myHtml += '<span id="bpc" title="Code: ' + CDG + '"><i class="fa fa-caret-right"></i></span> ';
			myHtml += 'Status del Controllo di Gestione: <span class="status" title="' + CDG + '">' + cdgStat[CDG] + '</status>';		// Status del BP
			myHtml += '</p>';
			myHtml += '<span id="infoTipoGestionale" class="hide">' + gestID + '</span>';
			myHtml += '</div>';
			...
			// Btn Importa
			if (btnImporta) {
				myHtml += '<a class="' + Class + ' disabled" href="#s" id="btn-importDP"><i class="far fa-save margin-r-5"></i> Importa dati previsionali</a>';
			}
			if (btnModifica) {
				myHtml += '<a class="' + Class + ' disabled" href="#s" id="btn-modifica"><i class="far fa-save margin-r-5"></i> Salva modifiche</a>';
			}
...
// *************************************
	...
	// RETURN:
	//  myHtml  : HTML formatted code as simple text (syncronous mode) or promise (asyncronous mode)
	function EX_buildHtml(tagPars, pbAttrs) {
		...
		return model.Common_Get({																		// Invia richiesta a SAAS
			...
			myHtml += 'Status del Controllo di Gestione: <span class="status" title="' + BP + '">' + bpStat[BP] + '</status>';		// Status del BP
			myHtml += '</p>';
			myHtml += '</div>';
			myHtml += '<div class="floatRight">';
...
// *************************************
	...
	function extend(params) {
		...
		var buttons = '#btn-save, #btn-modifica, #btn-importDP';										// ID dei pulsanti interessati dalla gestione dello stato "pagina modificata"
		...
		//	$('.hasChangedWithErr').removeClass('hasChangedWithErr').addClass('hasErr');
			...
			// Disabilita il pulsanti e rilancia il watcher in attesa di una nuova modifica
		...
		function watchSaveBtnState(){																	//  Gestisce il watcher del pulsante "Salva" per abilitarlo dopo una modifica dei dati
			...
			if (__SYS_status.hasChanged) {
				$(buttons).not('.btnInactive').removeClass('disabled');									// Abilita i pulsanti "Salva" e "Salva e compila", se non sono inattivi 
			} else {
